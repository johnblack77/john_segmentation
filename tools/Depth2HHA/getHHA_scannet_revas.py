...
str1 = '/Users/john/PycharmProjects/trail/label/data/scannet_frames_25k'
...
if __name__ == "__main__":
    ...
    for root, dirs, files in os.walk(str1):
        ll = [ele for ele in reversed(dirs)]
        dirs = ll
        for fi in dirs:
            path = root + '/' + fi
            for ro, di, fii in os.walk(path):
                if 'depth' in di:
                    xx = []
                    yy = []
                    for _,_,xx in os.walk(ro + '/depth'):
                        pass
                    if 'hha' in dirs:
                        for _,_,yy in os.walk(ro + '/hha'):
                            pass
                    ...
                    if len(xx) == len(yy):
                        ...
                        if len(yy) >0:
                            shutil.rmtree(ro + '/hha',True)

                        depth = ro + '/depth/'
                        intrin = ro + '/intrinsics_depth.txt'
